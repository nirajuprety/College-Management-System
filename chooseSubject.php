<?php

?>
<!DOCTYPE html>
<html lang="en">

<head>
    <meta charset="UTF-8">
    <meta http-equiv="X-UA-Compatible" content="IE=edge">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Document</title>
    <link rel="stylesheet" href="css/subject.css">
    <link href="assets/fontawesome/css/font-awesome.css" rel="stylesheet">


</head>

<body>
    <div class="topContainer">
        <div class="logo">
            <div class="imagediv">
                <a href="whyus.php"> <img src="img/logo.png" alt=""></a>
            </div>
            <div class="clz">
                <h1>College Space</h1>
            </div>
        </div>

    </div>
    <div class="container">
        <div class="leftContainer">
            <nav>
                <div class="nav-links">
                    <ul>
                    <li><a href="chooseSubject.php">Text Books</a> </li>
                        <li><a href="chooseSubject.php">Notes</a> </li>
                        <li><a href="addwebnotesform.php">Notices</a> </li>
                        <li><a href="addwebnotesform.php">Routines</a> </li>
                        <li><a href="addwebnotesform.php">Gallery</a> </li>
                        <li><a href="admission.php">Admission</a> </li>

                    </ul>
                </div>
            </nav>
        </div>
        <div class="rightContainer">
            <div class="grid-container">


                <div class="grid-item">
                    <a href="addwebnotesform.php">

                        <strong>Web Technology<br></strong>

                    </a>


                </div>
                <div class="grid-item">
                    <a href="addwebnotesform.php">
                        <strong>System Analysis and Dessign<br></strong>

                    </a>

                </div>
                <div class="grid-item">
                    <a href="addwebnotesform.php">
                        <strong>Design Analysis of Algorithm<br></strong>

                    </a>

                </div>
                <div class="grid-item">
                    <a href="addwebnotesform.php">
                        <strong>Simulation and Modeling<br></strong>

                    </a>
                </div>
                <div class="grid-item">
                    <a href="addwebnotesform.php">
                        <strong>Multimedia Computing<br></strong>

                    </a>

                </div>
                <div class="grid-item">
                    <a href="addwebnotesform.php">
                        <strong>Cryptography<br></strong>

                    </a>

                </div>


            </div>

        </div>
    </div>

    <footer>
           
           <div class="footer-leftContainer">
               <div>
                   <img src="img/logo.png">
               </div>
               <span class="footer-CS">
                   <h1>College Space</h1>
               </span>

           </div>
               
           </div>
           <div class="footer-middleContainer">
               <!-- links -->
               <div class="allLinks">
                   <ul>
                   <li> 
                           <a href="whyus.php">Whys Us</a>
                       </li><li> 
                           <a href="about.php">About Us</a>
                       </li><li> 
                           <a href="admission.php">Admission</a>
                       </li><li> 
                           <a href="faculty.html">Faculty</a>
                       </li>
                       <li> 
                           <a href="gallery.html">Gallery</a>
                       </li>
                   </ul>
               </div>
           </div>
           <div class="footer-leftContainer">
               <!-- social media -->
               <div class="icons">
               <div class="mail">
                       <span class="mailIcon">
                           <i class="fa fa-envelope-open"></i>
   
                       </span>
                       <span class="iconContent">
                        nirajuprety@gmail.com
   
                       </span>
                   </div>
                   <div class="phone">
                       <span class="phoneIcon">
                           <i class="fa fa-phone-square" aria-hidden="true"></i>
   
                       </span> +977 981600000
                   </div>
                   <div class="address">
                       <span class="addressIcon">
                           <i class="fa fa-map-marker" aria-hidden="true"></i>
   
                       </span> Kathmandu Gaushala
                   </div>
                   <div class="facebooklink">
                       <span class="facebookIcon">
                           <i class="fa fa-facebook-official" aria-hidden="true"></i>
   
                       </span>
                       <a href="https://www.facebook.com/Orchid-International-College-197982470220692">
                           Facebook
                       </a>

                   </div>
                   <div class="linkedinlink">
                       <span class="linkedinIcon">
                           <i class="fa fa-linkedin-square" aria-hidden="true"></i>
   
                       </span> LinkedIn
                   </div>
               </div>


           </div>

          
   </footer>

</body>

</html>